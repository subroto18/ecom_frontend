...
class UploadPhoto extends PureComponent {
            ...
            const checkVerify = Array.from(files).map((file,index) =>
                {
                    ...
                    const acceptedFileTypes = 'image/x-png, image/png, image/jpg, image/jpeg, image/webp'
                    const acceptedFileTypesArray = acceptedFileTypes.split(",").map((item) => {return item.trim()});
                    if (!acceptedFileTypesArray.includes(currentFileType)){

                        alert('warning',file.name+" file is not allowed. Only png , jpg, jpeg,gif are allowed!");

                        return false
                    }
                    else  if(file.size > 2097152){
                        alert('warning',"The file size should be less than 2 MB!");
                        return false;
                    }
                    else {
                        return true
    ...
    getPhoto(id) {
        ...
        Api().post('getFileById',data).then(res=>{
            this.setState({showFile: res.data,
                loading:false
            })
        })
    ...
    render() {
        ...
        const files = showFile.reverse().map(file => {
            return   <Col className="mb-4" xl={2} lg={2} md={3} sm={6} xs={12} key={file.id}>
                <div  className="card photo-card my-3">
                    <div className="card-body photo-card-body">
                        <Photo
                            src={this.props.backendApi+file.media_url}
        ...
        return (
            <Fragment>
                {this.state.files.length === 0 ?
                    <Dropzone className="dropzoneDiv"  multiple={this.props.multipleFile} accept="image/*" onDrop={this.handleOnDrop} maxSize={imageMaxSize}>
                    ...
                    :
                    <div>
                        <Dropzone multiple={this.props.multipleFile} accept="image/*" onDrop={this.handleOnDrop} maxSize={imageMaxSize}>
                            {({getRootProps, getInputProps}) => (
                                <section className="drag-image d-flex justify-content-center align-items-center align-middle">
                                    <div {...getRootProps({className: 'dropzone'})}>
