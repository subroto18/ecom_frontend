...
class ViewReturnOrdersPart extends PureComponent {
    constructor() {
        super();
        this.state = {
            loading: true,
    ...
    componentDidMount() {
        ...
        Api().post('getReturnOrderDetails', data).then(res => {
            this.setState({
                data: res.data,
                loading: false
            })
                                            ...
                                            product.status === "Pending" ?
                                                <div className="stepper-wrapper">
                                                    <div className="stepper-item completed active">
                                                        ...
                                                        <div className="step-name">Return initiated</div>
                                                    </div>
                                                    <div className="stepper-item  active">
                                                        <div className="step-counter">2</div>
                                                        <div className="step-name">Return Received</div>
                                                    </div>
                                            ...
                                            {product.product_price !== product.discount_price ?
                                                <Fragment>
                                                    {currencySymbolFormat === 1 ?
                                                        <p>{onCurrencyFormat(product.discount_price)}{defaultCurrency}
                                                            <del
                                                                className="ml-1 text-danger">{onCurrencyFormat(product.product_price)}{defaultCurrency}</del>
                                                            x {product.product_qnt} </p> :
                                                        <p>{defaultCurrency}{onCurrencyFormat(product.discount_price)}
                                                            <del
                                                                className="ml-1 text-danger">{defaultCurrency}{onCurrencyFormat(product.product_price)}</del>
                                                            x {product.product_qnt} </p>
                                                    }
                                                </Fragment>
                                                :
                                ...
                                <Fragment>
                                    {product.status === "Rejected" &&
                                        <div className="orderDiv">
                                            ...
                                            <p className="alert alert-info mt-3">If you have any inquiries regrading
                                                this rejection please talk to our <Link href="/support-ticket"><span
                                                    className="badge badge-dark">support</span></Link></p>
                                        </div>
                                    }
                                </Fragment>
                            </Fragment>
...
export default connect(mapStateToProps)(ViewReturnOrdersPart);
